...
def parse_args():
    ...
    parser.add_argument("--tick_rate", type=float, default=3.0)
    ...
    parser.add_argument("--resume_step", type=int, default=120000)
...
# ===============================================================
class CarController:
    def __init__(self, car_ip: str, car_port: int = 5000):
        self.car_ip = car_ip
        self.car_port = car_port
        self.base_url = f"http://{car_ip}:{car_port}"
        print(f"[CarController] URL: {self.base_url}")

    def send_velocity(self, linear: float, angular: float) -> bool:
        try:
            url = f"{self.base_url}/cmd_vel"
            ...
            try:
                url = f"{self.base_url}/cmd_vel?linear={linear}&angular={angular}"
...
class Inference:
    def __init__(self, args, vla_model, action_head_model, pose_projector_model,
        ...
        self.car = CarController(args.car_ip, args.car_port)
    ...
    # Transform Data
    # ----------------------------
    def transform_datatype(self, inst_obj, actions, goal_pose_cos_sin, current_image_PIL, goal_image_PIL, prompt_builder,
    ...
    def data_transformer_omnivla(self, current_image_PIL, lan_inst, goal_image_PIL,
                                 goal_pose_loc_norm, prompt_builder, action_tokenizer, processor):
    ...
    def run_forward_pass(self, vla, action_head, noisy_action_projector, pose_projector,
        ...
        if     s and not l and not p and not i: mid = 0
        elif   s and not l and     p and not i: mid = 1
        elif   s and not l and not p and     i: mid = 2
        elif   s and not l and     p and     i: mid = 3
        elif not s and not l and     p and not i: mid = 4
        elif not s and not l and     p and     i: mid = 5
        elif not s and not l and not p and     i: mid = 6
        elif not s and     l and not p and not i: mid = 7
        elif not s and     l and     p and not i: mid = 8
        else: mid = 7
